import { generateTerraform } from "../src/terraform-generator.mjs";
import { validateArchitecture } from "../src/architecture-validator.mjs";
...
async function testSafetyApplyBlocked() {
  ...
  const { sanitizeCommandArgs, FORBIDDEN_COMMANDS } = await import("../src/terraform-executor.mjs");
  let blocked = false;
  try {
