...
| 9 | create_ticket | Submit a service ticket | write | high | explicit_approval=true required |
...
# Create ticket (REAL submission — requires explicit approval)
# Tool: create_ticket
# Parameters: { payload: { ... }, explicit_approval: true }
# If explicit_approval is missing, false, or non-boolean, the request is rejected.
```
...
1 approval guard test: test-approval-guard.mjs

## Security

- `create_ticket` creates a **real** service ticket — requires `explicit_approval=true` and use `prepare_ticket` first [VERIFIED_FROM_CODE]
- `create_ticket` rejects requests where `explicit_approval` is missing, false, or non-boolean [VERIFIED_FROM_CODE]
