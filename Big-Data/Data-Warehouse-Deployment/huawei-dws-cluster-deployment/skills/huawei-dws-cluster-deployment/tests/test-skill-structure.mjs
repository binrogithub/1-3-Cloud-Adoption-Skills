...
test('example file exists', () => {
  assert(fileExists('examples/dws-cluster-deployment.example.md'), 'Missing example file');
});
...
test('mcp extension request exists', () => {
  assert(fileExists('docs/dws-mcp-extension-request.md'), 'MCP extension request doc missing');
});

test('mcp extension tools are NOT_IMPLEMENTED', () => {
  const content = readFile('docs/dws-mcp-extension-request.md');
...
test('examples sanitized', () => {
  const content = readFile('examples/dws-cluster-deployment.example.md');
  const uuidPattern = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i;
