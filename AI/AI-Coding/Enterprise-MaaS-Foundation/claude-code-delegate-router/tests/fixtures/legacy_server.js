...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && (url.pathname === "/" || url.pathname === "/health")) {
    sendJson(res, 200, { status: "ok", upstream: MAAS_CHAT_URL, model: DEFAULT_MODEL });
    return;
  }
...
server.listen(PORT, HOST, () => {
  console.log(`claude-code-maas proxy listening on http://${HOST}:${PORT} -> ${MAAS_CHAT_URL} (model: ${DEFAULT_MODEL})`);
});
