...
Related: [`litellm-maas-auto-plugin/docs/PRD-lean-glm51.md`](../../litellm-maas-auto-plugin/docs/PRD-lean-glm51.md) (gateway policy and plugin baseline). The removed Claude Code forky router is retained here only as a historical transport-layer comparison.

## 1. Goal

One entry point — plain `claude` with the user's Claude.ai OAuth subscription, transport completely untouched — that:

- executes **premium-class** work in-session on Claude (architecture planning, complex debugging, security review, production incidents, high-risk PR review, image/screenshot input, >198K raw context);
...
```
 user ──► claude  (official client, OAuth, ZERO middleware) ──────────► api.anthropic.com
            ...
            │                                   │           smart_router + validated rules
            │                                   │           rolling_budget_hook
            │                                   ├─ execution / ≤198K → GLM-5.1
...
| | Gateway smart router (`smart_router`) | removed forky transport router | **This PRD** |
|---|---|---|---|
...
| Protocol patches needed | stream + reasoning guards (GLM side) | 3 forky patches + rebase burden | stream + reasoning guards (GLM side) |
...
Taxonomy inherited from the current gateway PRD, re-anchored to the orchestration layer.
...
| >198K raw context | task exceeds the configured GLM boundary and resists splitting |
...
A structural advantage over gateway classification: the orchestrator classifies with **first-hand repo signals** — it reads the changed files, git history, labels, CODEOWNERS itself — where a gateway must infer the same from prompt text and caller-supplied metadata.
...
The GLM path is a **native Claude Code client speaking unmodified Anthropic protocol to LiteLLM**. Keep GLM thinking enabled. `anthropic_stream_guard` keeps requests on `/chat/completions`, repairs malformed SSE block families, synthesizes terminal events, amplifies interjections, and detects raw tool markup. `anthropic_reasoning_filter` hides internal thinking only from Anthropic/Claude responses while preserving final text, structured `tool_use`, usage, and termination events; OpenAI-compatible clients retain `reasoning_content`. `smart_router` enforces the 198K and Vision/Premium policies. This is the server-side fix for previously observed garbled reasoning output; no client-side patch is required.
...
|---|---|---|
| C1 | `claude-glm` wrapper | `~/.local/bin/claude-glm`: sets `CLAUDE_CONFIG_DIR=$HOME/.claude-glm`, `ANTHROPIC_BASE_URL=http://127.0.0.1:4000`, `ANTHROPIC_API_KEY=<virtual key>`, `ANTHROPIC_MODEL` + `ANTHROPIC_SMALL_FAST_MODEL` → LiteLLM aliases, `CLAUDE_CODE_AUTO_COMPACT_WINDOW=198000`, and `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`. Env scoped **inside the wrapper only** — never a shell profile. Key pre-approved in the isolated `.claude.json` (reuse `configure-claude-code.sh --print-env` to generate; run under the isolated config dir). |
| C2 | `delegate` script | Thin runner around `claude-glm -p`: builds the brief, sets headless permissions (`--permission-mode acceptEdits`, tool allowlist, workspace-scoped), timeout, retries ≤ 2, writes an audit record (§10), and returns structured JSON. The second attempt is a fresh delegation containing concrete failure evidence. |
...
| C6 | Server plugins | `anthropic_stream_guard`, `anthropic_reasoning_filter`, and `smart_router` with schema-validated multilingual rules (required), plus `rolling_budget_hook` with `BUDGET_TIER_KEY` on the delegate key. Installed by `server/install-litellm-plugin.sh`. Configure GLM execution, `premium-openrouter`, `vision-openrouter`, and `vision-openrouter-secondary`; scope the delegate key to every fallback model. **The `claude-*` wildcard route remains mandatory for workflows** because it maps internal subagent aliases onto GLM-5.1 (§6.3 W1). |
...
The brief is the canonical delegated context; the summary is the bounded attempt record.
...
- Server prerequisites: wildcard route plus all three auto-plugin callbacks and rules. `CLAUDE_CODE_AUTO_COMPACT_WINDOW=198000` matches the configured GLM boundary; `smart_router` sends input above that boundary to Premium.
...
Applies the gateway fallback and stabilization principles at task granularity:

1. Delegate attempt 1 → on failure, delegate attempt 2 **with the failure evidence appended to the brief** as a fresh delegated attempt.
...
4. **No oscillation is possible** by construction: the OAuth session never "downgrades" — it only chooses per new task.
...
7. **Workflow remainder rule**: failed manifest items collect into the premium remainder, which the orchestrator finishes in-session after the batch completes. If the remainder exceeds ~30% of items, abort the workflow and reclassify — GLM is not a blanket replacement for Premium.

## 8. Context Policy

| Range (brief size) | Policy |
|---|---|
| ≤ 198K estimated tokens | delegate directly when the brief is self-contained |
| > 198K but reducible | compress the brief through file selection and summarization |
| > 198K and unsplittable | premium in-session (`route_reason=context_above_glm_limit`) |

- Images/screenshots normally stay in the OAuth Premium session. If one reaches
  the delegate gateway, `smart_router` sends it to `vision-openrouter`, whose
  only fallback is `vision-openrouter-secondary`.
- Server-side `smart_router` sends estimated input above 198000 to Premium.
...
- GLM side: `asg_*`, `smart_router_*`, spend logs, and
  `metadata.smart_router` routing/fallback fields.
- A `route-stats` script aggregates GLM coverage ratio, escalation rate, and
  cost per accepted task so dashboards stay comparable.
...
- **C (protocol)**: `/effort max` on `claude-glm` completes without 400/429 or garbled internal reasoning; Claude receives final text, structured `tool_use`, and terminal events but no thinking blocks; an OpenAI-compatible probe still receives `reasoning_content`. Confirm stream-repair counters as applicable and assert no `ANTHROPIC_*` overrides leaked into the plain OAuth environment.
...
| Sub-orchestrator context overflow | context errors above 198K | `CLAUDE_CODE_AUTO_COMPACT_WINDOW=198000` plus smart-router Premium routing |
| MaaS rate limits under fan-out | 429 storms | concurrency below key RPM/TPM; honor `Retry-After`, bounded exponential backoff with jitter, and two attempts maximum |
...
`server/install-litellm-plugin.sh` + all three LiteLLM callbacks and rules, and the spend-log
SQL verification pattern used by the LiteLLM MaaS plugin probes.

Env table (wrapper-internal only): `CLAUDE_CONFIG_DIR`, `ANTHROPIC_BASE_URL`, `ANTHROPIC_API_KEY` (virtual key), `ANTHROPIC_MODEL=claude-opus-4-6` (LiteLLM alias → glm-5.1), `ANTHROPIC_SMALL_FAST_MODEL=claude-glm1`, `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`. Delegate runner: brief `max_attempts` is hard-capped to `1..2`; `DELEGATE_TIMEOUT_S=1800`, `DELEGATE_MAX_BACKOFF_S=30`, `DELEGATE_AUDIT=~/.claude-hybrid/route-audit.jsonl`.
...
## Appendix B — Relationship to the gateway smart router

This PRD implements the same taxonomy with the interactive Premium pool provided
by the user's OAuth session. The shared gateway still exposes GLM execution,
`premium-openrouter`, and separate primary/secondary Vision groups for
delegated or non-OAuth clients. Repository-risk signals are observed directly
by the orchestrator, while the gateway uses prompt text and caller metadata.
Both designs share the LiteLLM plugin layer and KPI definitions.
...
| live_smoke text / tools / big (185K ctx) | all HTTP 200 below the 198K boundary |
...
`generate_config.sh`); `asg_*`/`smart_router_*` Prometheus counters register lazily
