...
```bash
../litellm-maas-auto-plugin/server/install-litellm-plugin.sh
```

Then ensure (manually, or confirm the installer applied them):

- `anthropic_stream_guard`, `anthropic_reasoning_filter`, and `smart_router`
  mounted and registered in that order, with `smart_router_rules.json`.
- `use_chat_completions_url_for_anthropic_messages: true` in `litellm_settings`.
- GLM execution, `premium-openrouter`, `vision-openrouter`, and
  `vision-openrouter-secondary` model groups configured. A **`claude-*` wildcard
  route** points at GLM-5.1:
...
liveness 200; a `/v1/messages` request with a `thinking` param returns final
text without exposing internal reasoning; a non-alias name such as
`claude-haiku-4-5-20251001` gets served. Confirm an OpenAI-compatible OpenCode
request can still receive `reasoning_content`.
...
curl -s http://localhost:4000/key/generate -H "Authorization: Bearer $LITELLM_MASTER_KEY" \
  ...
  -d '{"key_alias":"delegate-<host>",
       "models":["claude-*","premium-openrouter","vision-openrouter",
                 "vision-openrouter-secondary"],
       "metadata":{"purpose":"oauth-delegate-router execution pool"}}'
```

Give CI/loops their own key. Budget circuit breaker: set `BUDGET_TIER_KEY`
(rolling-window, e.g. `5h:12`) in the stack `.env`. Scope the key to
`claude-*`, `premium-openrouter`, `vision-openrouter`, and
`vision-openrouter-secondary`.
...
- **429 storms under fan-out** — concurrency exceeds key rpm; lower `concurrency`,
  honor `Retry-After`, use bounded backoff with jitter, and never raise the
  two-attempt ceiling.
- **Garbled strings or visible internal reasoning** — verify
  `anthropic_reasoning_filter` is registered after `anthropic_stream_guard`.
  Keep GLM thinking enabled; hide it only on the Anthropic/Claude response
  boundary.
- **`asg_*`/`smart_router_*` counters absent from `/metrics`** — lazy registration; they
