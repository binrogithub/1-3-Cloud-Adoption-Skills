...
`install <github-tree-url>` in Codex downloads the entire referenced
directory into `~/.codex/skills/<name>/` and looks for a `SKILL.md` at
that directory root. Pointing it at this repo's tree URL —

```
https://github.com/binrogithub/1-3-Cloud-Adoption-Skills/tree/main/AI/AI-Coding/AI-Coding-Best-Practice/ai-dlc
```

— installs the **whole toolkit** (`bin/plan.py`, `bin/report.py`,
`config/`, `openspec/`, …) into `~/.codex/skills/ai-dlc/`. The root
`SKILL.md` (generated by `./install.sh --gen-root-skill`) tells the agent
to run those tools from the skill's own directory with `--repo` pointing
at whatever project is actually under development.

This is different from `--target codex --target-dir <path>`, which only
writes an `AGENTS.md` instruction file into a project you already have
