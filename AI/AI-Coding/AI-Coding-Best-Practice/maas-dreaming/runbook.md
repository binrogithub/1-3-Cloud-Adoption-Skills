...
```bash
cd .../enterprise-context-engineering/maas-dreaming
...
```bash
python3 scripts/build_skill.py --output dist/maas-dreaming
```
...
```text
/schedule create "maas-dreaming nightly" --cron "0 2 * * *" \
...
```cron
0 2 * * *  cd /path/to/maas-dreaming && MCE_APPLY=1 bash bin/dream-nightly.sh /path/to/your/repo >> ~/.maas-dreaming.log 2>&1
```
...
The nightly wrapper ports MiMo `auto-dream.ts` skip gates: it skips projects
...
In Claude Code, invoke the skill. The default `/maas-dreaming` path writes a
review-only dreaming summary report; clear/reset is a separate explicit command.
Equivalent commands:

```bash
# default /maas-dreaming: report only; no nested LLM process
...
```bash
# /maas-dreaming clear: dry-run, reports what would be removed
python3 scripts/reset_memory.py --repo-root .

# /maas-dreaming clear --apply: backup, then remove selected memory artifacts
...
native memory directory so later maas-dreaming runs do not fall back to the
...
Scan a directory into a dream source file, then run the MiMo prompt over it:
...
## Conflict detection — how it decides (5.3)
...
| `/maas-dreaming clear` refuses parent memory | pass the exact project directory as `--repo-root`, or use `--allow-parent` only if clearing the parent workspace is intentional |
...
## Scope
Only memory **hygiene** (5.3 + 5.4) + recall (5.5). L0 layering (5.1) and the
tier/index *format* (5.2) are native Claude Code behavior and were removed — see
