...
if [ "$TARGET" = "claude" ] || [ "$TARGET" = "both" ]; then
  INSTALLED="$HOME/.claude/skills/maas-dreaming"
  if [ -d "$INSTALLED" ]; then
...
**File:** `SKILL.md` — Default `/maas-dreaming` behavior section
...
**How:** Add two lines to the "Default `/maas-dreaming` behavior" section:
```
...
to sync the installed bundle at `$HOME/.claude/skills/maas-dreaming/`.
```
**Acceptance:**
