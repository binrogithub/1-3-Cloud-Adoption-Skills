...
def dream_prompt_path() -> Path:
    ...
    maas-dreaming should stay a thin adapter around MiMo dream semantics. The
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.claude_bin:
        print("error: external LLM runner requires --claude-bin. Default /maas-dreaming uses the host agent via dream_agent_report.py.", file=sys.stderr)
        return 2
