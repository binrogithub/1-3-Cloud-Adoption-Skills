    ...
    for dirpath, dirnames, filenames in os.walk(str(repo_root)):
        ...
        dirnames[:] = [
            ...
            if d not in {".git", ".hg", "node_modules", "vendor", "build",
                         "dist", "__pycache__", ".venv", ".maas-dreaming",
...
def _cold_start_report(repo_root: Path, instructions: str = "") -> str:
    ...
    if is_git:
        lines.extend([
            ...
            "```",
            "",
            "After indexing, run `/maas-dreaming` again to get a full dream report",
            "with project overview, key symbols, coupling insights, and recent activity.",
            "",
        ])
    else:
        lines.extend([
            "maas-dreaming works best with either:",
            "",
            "1. **Session trajectory** — start a Claude Code session and run",
            "   `/maas-dreaming` after working for a while.",
...
def render_report(
    ...
    code_index_db:
        ...
        ``<repo_root>/.maas-dreaming/code-index.db`` path is used.
    """
    repo_root = Path(repo_root)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--db",
        default=None,
        help="Path to the code-index SQLite database (default: <repo-root>/.maas-dreaming/code-index.db).",
    )
