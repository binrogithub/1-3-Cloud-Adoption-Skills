"""SQLite-backed code structure index for maas-dreaming.
...
class CodeIndex:
    ...
    Typical usage::
        ...
        with CodeIndex.open_or_create(Path(".maas-dreaming/code-index.db")) as idx:
...
_GITIGNORE_ENTRIES = [
    ".maas-dreaming/code-index.db",
    ".maas-dreaming/code-index.db-wal",
    ".maas-dreaming/code-index.db-shm",
]
...
def setup_git_merge_strategy(repo_root: Path) -> None:
    ...
    - Writes/updates ``.maas-dreaming/.gitattributes`` with ``merge=ours``
      entries for ``code-index.db.zst`` and ``artifact.json``.
    - Appends ``.maas-dreaming/code-index.db`` (and WAL/SHM sidecar files)
    ...
    # --- .gitattributes ---
    ga_dir = repo_root / ".maas-dreaming"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Build or update the maas-dreaming code structure index."
    )
    parser.add_argument(
        "--repo-root",
        default=".",
    ...
    parser.add_argument(
        "--db",
        default=".maas-dreaming/code-index.db",
