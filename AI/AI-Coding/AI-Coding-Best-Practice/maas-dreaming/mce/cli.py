"""Single entry point that dispatches to the reused scripts / glue.
    ...
    python -m mce.cli artifact export [--db .maas-dreaming/code-index.db]
                                      [--output .maas-dreaming/code-index.db.zst]
    python -m mce.cli artifact import [--input .maas-dreaming/code-index.db.zst]
...
def _cmd_artifact(rest: list) -> int:
    ...
    if subcmd == "export":
        ...
        ap.add_argument(
            ...
            help="Path to the SQLite code-index DB (default: .maas-dreaming/code-index.db)",
        )
        ap.add_argument(
            "--output",
            default=None,
    ...
    if subcmd == "import":
        ...
        ap.add_argument(
            "--input",
            default=".maas-dreaming/code-index.db.zst",
            help="Source .zst artifact (default: .maas-dreaming/code-index.db.zst)",
        )
        ap.add_argument(
            "--db",
            default=".maas-dreaming/code-index.db",
            help="Destination DB path (default: .maas-dreaming/code-index.db)",
        )
        a = ap.parse_args(sub_rest)
