# PRD v2: maas-dreaming as Project Knowledge Continuous Maintenance Engine
...
maas-dreaming is a memory hygiene skill that deduplicates, validates, and
...
Transform maas-dreaming into a **two-layer project knowledge engine**:
...
**Key property:** A brand-new session on any project can invoke `/maas-dreaming`
...
```
/maas-dreaming (default invocation)
    |
    v
...
[Phase 1: Code Structure Index] .............. DETERMINISTIC, NO LLM
    ...
    |       Output: .maas-dreaming/code-index.db
    |
    v
...
[Phase 3: Host Agent Synthesis] .............. LLM (HOST AGENT)
    ...
    v
[Output Artifacts]
    .maas-dreaming/code-index.db ........... SQLite knowledge graph
    .maas-dreaming/code-index.manifest.json  Incremental fingerprints
    .maas-dreaming/DREAMS.md ............... Report index
...
idx = CodeIndex(".maas-dreaming/code-index.db")
...
The `.maas-dreaming/` directory follows the existing `--output-mode project-root`
...
1. Developer A runs `/maas-dreaming` -> full index -> exports `code-index.db.zst`
2. Commits `.maas-dreaming/` directory
3. Developer B clones -> `/maas-dreaming` detects artifact -> decompresses ->
...
### 5.1 Changes to Default `/maas-dreaming` Behavior

The default invocation now runs a **three-phase pipeline**:

```
Step 1: Code index (deterministic)
    python3 scripts/code_index.py --repo-root REPO --db .maas-dreaming/code-index.db

Step 2: Dream report (enhanced)
    python3 scripts/dream_agent_report.py --repo-root REPO --code-index .maas-dreaming/code-index.db
...
Code index created: .maas-dreaming/code-index.db (312 symbols, 142 commits)
...
```bash
# New subcommand
python -m mce.cli index --repo-root . [--db .maas-dreaming/code-index.db]
...
code_index.py --repo-root . --db .maas-dreaming/code-index.db
dream.py --memory-dir DIR --repo-root . [--apply]
dream_agent_report.py --repo-root . --code-index .maas-dreaming/code-index.db
...
The default `/maas-dreaming` section in SKILL.md changes to:

```markdown
## Default `/maas-dreaming` behavior

The default `/maas-dreaming` invocation runs a three-phase pipeline:

1. **Code index** (deterministic, no LLM):
   ```bash
   python3 BASE/scripts/code_index.py --repo-root REPO --db REPO/.maas-dreaming/code-index.db
   ```
...
2. **Dream report** (deterministic + host-agent synthesis):
   ```bash
   python3 BASE/scripts/dream_agent_report.py --repo-root REPO --code-index REPO/.maas-dreaming/code-index.db
   ```
...
- The `.maas-dreaming/` directory is clearly separated from project source
...
2. `.maas-dreaming/.gitignore` generation
...
- **Real-time file watching** — unlike CodeGraph's MCP server, maas-dreaming is
