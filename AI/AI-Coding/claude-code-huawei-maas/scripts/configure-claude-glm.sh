...
MAAS_AUTO_COMPACT_WINDOW="${MAAS_AUTO_COMPACT_WINDOW:-$MAAS_CONTEXT_TOKENS}"
...
# CLAUDE_CODE_MAX_CONTEXT_TOKENS is ignored by Claude Code unless DISABLE_COMPACT
# is set; CLAUDE_CODE_AUTO_COMPACT_WINDOW is the working knob: it fixes the
...
# the authenticated /v1/messages endpoint once (1 max_token) and fail fast
# on auth errors only; any other status (200/400/5xx/000 timeout) passes
